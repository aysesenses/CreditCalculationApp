...
class HouseLoanOfferViewModel : ViewModel() {
    ...
    fun getInstallment(loan_amount: Int, maturity : Int, interest: Double): Double{
        val bsmv  = 0.00
        val kkdf  = 0.00
        val taxInterest = (interest / 100) * (1 + bsmv + kkdf )

        val newMaturity = maturity * 12

        val k1 = taxInterest * Math.pow((1 + taxInterest), newMaturity.toDouble())
        val k2 = Math.pow((1 + taxInterest), newMaturity.toDouble()) - 1
